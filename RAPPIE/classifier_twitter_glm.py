...
def main():
    ...
    def batch_loader(data, data_type):
        ...
        if not os.path.exists(args.dataset + "_y/" + data_type + "/batch_0.pkl"): # 全部关系
            print(args.dataset + "_y/" + data_type + "/batch_0.pkl")
    ...
    def evaluate(args, is_test):
        if is_test:
            dataset = read_dataset(args.dataset + "/Test_yw.txt")[1:]
            data_type = "test"
        else:
    ...
    path2 = "dataset/twitter_glm/user_embedding_repost_r.pkl"
    path3 = "dataset/twitter_glm/user_embedding_comment_r.pkl"
    file2 = open(path2, "rb")
    file3 = open(path3, "rb")
    role_file = open("dataset/twitter_glm/role_features.pkl", "rb")
