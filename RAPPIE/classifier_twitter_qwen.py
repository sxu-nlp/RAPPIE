...
class ChatglmClassifier(nn.Module):
    ...
    def __init__(self, args):  # 定义类的初始化函数，用户传入的参数
        ...
        self.labels_num = 7
        self.linear_adapter_i = torch.nn.Linear(5120, 2560)
        self.linear_adapter_r = torch.nn.Linear(5120, 2560)
        self.linear_adapter_u = torch.nn.Linear(5120, 2560)
...
def main():
    ...
    def batch_loader(data, data_type):
        if not os.path.exists(args.dataset + "_"+'qwen_' + "_y/" + data_type + "/batch_0.pkl"): # 全部关系
            print(args.dataset + "_"+'qwen_' + "_y/" + data_type + "/batch_0.pkl")
            dataset = []
            random.shuffle(data)
            batch_id = 0
            for line_id, line in enumerate(data):
                if len(dataset) == batch_size or line_id == len(data)-1:
                    batch_file = args.dataset + "_"+'qwen_'+ "_y/" + data_type + "/batch_" + str(batch_id) + '.pkl' # 全部关系
                ...
                text_id = int(line[columns["text_id"]]) - 1
                # 用户关系
                user_r2 = user_r2_emb[user_id]  # 转发
                user_r3 = user_r3_emb[user_id]  # 评论
                user_r = torch.stack([user_r2, user_r3])  # 全部融合/去传播角色
                role = role_emb
                inputs = text_emb[text_id]
        ...
        for i in range(math.ceil(len(data)/batch_size)):
            with open(args.dataset + "_"+'qwen_' + "_y/" + data_type + "/batch_" + str(i) + ".pkl", "rb") as f:# 全部关系
    ...
    def evaluate(args, is_test):
        if is_test:
            dataset = read_dataset(args.dataset + "/Test_n_yw.txt")[1:]
            data_type = "test"
        else:
            dataset = read_dataset(args.dataset + "/Vaild_yw.txt")[1:]
            data_type = "evaluate"
        ...
        for i, (inputs_batch, label_batch, user_r_batch, batch_role) in enumerate(
                batch_loader(dataset, data_type)):  # 循环
    ...
    path2 = "dataset/twitter_qwen/user_embedding_retweet_r.pkl"
    path3 = "dataset/twitter_qwen/user_embedding_comment_r.pkl"
    file2 = open(path2, "rb")
    file3 = open(path3, "rb")
    role_file = open("dataset/twitter_qwen/role_features_qwen.pkl", "rb")
    text_file = open("dataset/twitter_qwen/twitter_text_features.pkl", "rb")
    ...
    text_emb = torch.from_numpy(pickle.load(text_file)).cpu().detach()
    file2.close()
    file3.close()
    role_file.close()
    text_file.close()
    dataset = read_dataset(args.dataset + "/Train_en_yw.txt")
